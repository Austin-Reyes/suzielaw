...
import type { PersonasDB } from '../src/types.js';

const POSTGRES_IMAGE = 'pgvector/pgvector:pg16';
const here = dirname(fileURLToPath(import.meta.url));
const migrationsDir = resolve(here, '..', '..', 'db', 'migrations');

export interface PerTestEnv {
  kysely: Kysely<PersonasDB>;
  pool: Pool;
  stop: () => Promise<void>;
}

export async function startPerTestEnv(): Promise<PerTestEnv> {
  ...
  const kysely = new Kysely<PersonasDB>({ dialect: new PostgresDialect({ pool }) });
