...
import type { GridReviewDB } from '../src/types.js';

const POSTGRES_IMAGE = 'pgvector/pgvector:pg16';
const here = dirname(fileURLToPath(import.meta.url));
const migrationsDir = resolve(here, '..', '..', 'db', 'migrations');

export interface GrTestEnv {
  kysely: Kysely<GridReviewDB>;
  pool: Pool;
  stop: () => Promise<void>;
}

export async function startGrTestEnv(): Promise<GrTestEnv> {
  ...
  const kysely = new Kysely<GridReviewDB>({ dialect: new PostgresDialect({ pool }) });
