...
import type { WorkflowsDB } from '../src/types.js';

const POSTGRES_IMAGE = 'pgvector/pgvector:pg16';
const here = dirname(fileURLToPath(import.meta.url));
const migrationsDir = resolve(here, '..', '..', 'db', 'migrations');

export interface WfTestEnv {
  kysely: Kysely<WorkflowsDB>;
  pool: Pool;
  stop: () => Promise<void>;
}

export async function startWfTestEnv(): Promise<WfTestEnv> {
  ...
  const kysely = new Kysely<WorkflowsDB>({ dialect: new PostgresDialect({ pool }) });
