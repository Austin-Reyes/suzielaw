...
import type { ModelSettingsDB } from '../src/types.js';

const POSTGRES_IMAGE = 'pgvector/pgvector:pg16';
const here = dirname(fileURLToPath(import.meta.url));
const migrationsDir = resolve(here, '..', '..', 'db', 'migrations');

export interface MsTestEnv {
  kysely: Kysely<ModelSettingsDB>;
  pool: Pool;
  stop: () => Promise<void>;
}

export async function startMsTestEnv(): Promise<MsTestEnv> {
  ...
  const kysely = new Kysely<ModelSettingsDB>({ dialect: new PostgresDialect({ pool }) });
