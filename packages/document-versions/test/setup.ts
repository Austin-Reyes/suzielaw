...
import type { DocumentVersionsDB } from '../src/types.js';

const POSTGRES_IMAGE = 'pgvector/pgvector:pg16';
const here = dirname(fileURLToPath(import.meta.url));
const migrationsDir = resolve(here, '..', '..', 'db', 'migrations');

export interface DvTestEnv {
  kysely: Kysely<DocumentVersionsDB>;
  pool: Pool;
  stop: () => Promise<void>;
}

export async function startDvTestEnv(): Promise<DvTestEnv> {
  ...
  const kysely = new Kysely<DocumentVersionsDB>({ dialect: new PostgresDialect({ pool }) });
