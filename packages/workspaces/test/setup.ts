...
import type { WorkspacesDB } from '../src/types.js';

const POSTGRES_IMAGE = 'pgvector/pgvector:pg16';
const here = dirname(fileURLToPath(import.meta.url));
const migrationsDir = resolve(here, '..', '..', 'db', 'migrations');

export interface WorkspacesTestEnv {
  kysely: Kysely<WorkspacesDB>;
  pool: Pool;
  stop: () => Promise<void>;
}

export async function startWorkspacesTestEnv(): Promise<WorkspacesTestEnv> {
  ...
  const kysely = new Kysely<WorkspacesDB>({ dialect: new PostgresDialect({ pool }) });
