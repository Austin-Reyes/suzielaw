...
import type { ChatsDB } from '../src/types.js';

const POSTGRES_IMAGE = 'pgvector/pgvector:pg16';
const here = dirname(fileURLToPath(import.meta.url));
const migrationsDir = resolve(here, '..', '..', 'db', 'migrations');

export interface ChatsTestEnv {
  kysely: Kysely<ChatsDB>;
  pool: Pool;
  stop: () => Promise<void>;
}

export async function startChatsTestEnv(): Promise<ChatsTestEnv> {
  ...
  const kysely = new Kysely<ChatsDB>({ dialect: new PostgresDialect({ pool }) });
