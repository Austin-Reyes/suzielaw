...
import type { SharingDB } from '../src/types.js';

const POSTGRES_IMAGE = 'pgvector/pgvector:pg16';
const here = dirname(fileURLToPath(import.meta.url));
const migrationsDir = resolve(here, '..', '..', 'db', 'migrations');

export interface ShTestEnv {
  kysely: Kysely<SharingDB>;
  pool: Pool;
  stop: () => Promise<void>;
}

export async function startShTestEnv(): Promise<ShTestEnv> {
  ...
  const kysely = new Kysely<SharingDB>({ dialect: new PostgresDialect({ pool }) });
